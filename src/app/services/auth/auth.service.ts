...
export class AuthService {
  ...
  LoginUser(user: loginUser) : Observable<loginUser>{
    return this._http.post<loginUser>(`${environment.API_URL}/auth/login`,user, {
      withCredentials: true
    })
  }

  registerUser(user:registerUser) : Observable<registerUser>{
    return this._http.post<registerUser>(`${environment.API_URL}/auth/signup`,user, {
      withCredentials: true
    })
  }

  logoutUser(): Observable<void> {
    return this._http.post<void>(`${environment.API_URL}/auth/logout`, {}, {
      withCredentials: true
    });
  }

  authenticateUser(): Observable<boolean> {
    return this._http.get(`${environment.API_URL}/auth/authenticate`, {
      withCredentials: true
    }).pipe(
